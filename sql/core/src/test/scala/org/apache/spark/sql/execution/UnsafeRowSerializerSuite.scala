...
class UnsafeRowSerializerSuite extends SparkFunSuite with LocalSparkSession {
  ...
  test("streaming shuffle ByteBuf codec preserves UnsafeRow stream format") {
    ...
    Seq(Unpooled.directBuffer(), Unpooled.buffer()).foreach { buffer =>
      try {
        ...
        val decoded = serializer.keyValueIteratorFromByteBuf(buffer).map {
          case (_, row: UnsafeRow) => row.copy()
        }.toSeq
