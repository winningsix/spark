  ...
  override def apply(plan: SparkPlan): SparkPlan = {
    ...
    val optimizedPlan = plan.transformUp {
      case smj @ SortMergeJoinExec(leftKeys, rightKeys, joinType, condition, left, right, false)
        ...
        val buildSide = conf.convertSortMergeJoinToShuffledHashJoinForceBuildSide match {
          case "left" =>
            Option.when(canBuildShuffledHashJoinLeft(smj.joinType))(BuildLeft)
          case "right" =>
            Option.when(canBuildShuffledHashJoinRight(smj.joinType))(BuildRight)
          case "none" =>
            ...
            // widen each row, so its second argument must be the input shuffle stage's output (not
            // the join child's own output, which would always yield a ratio of 1.0). Compute it
            // only when the stage is known-defined.
