...
private class UnsafeRowSerializerInstance(
  ...
  override def writeValueToByteBuf(value: Any, output: ByteBuf): Unit = {
    ...
    if (output.hasMemoryAddress) {
      ...
      row.writeToMemory(
        output.array(),
        Platform.BYTE_ARRAY_OFFSET + output.arrayOffset() + targetIndex)
      output.writerIndex(targetIndex + rowSize)
    } else {
      // Composite buffers are not expected on the writer data path. Preserve correctness if a
      // different allocator supplies one, at the cost of a temporary byte array.
    ...
    new Iterator[(Any, Any)] {
      ...
      override def next(): (Any, Any) = {
        ...
        if (input.hasMemoryAddress) {
          ...
          row.pointTo(
            input.array(),
            Platform.BYTE_ARRAY_OFFSET + input.arrayOffset() + rowIndex,
            rowSize)
        }
