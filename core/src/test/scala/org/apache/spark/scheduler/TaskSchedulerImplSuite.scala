...
class TaskSchedulerImplSuite extends SparkFunSuite with LocalSparkContext
  ...
  test("reader frontier reservation survives partial completion before the next stage is ready") {
    val taskScheduler = setupScheduler(
      config.STREAMING_SHUFFLE_EXECUTOR_RECEIVE_SERVICE_ENABLED.key -> "true",
    ...
    val endpoint = sc.env.rpcEnv.setupEndpoint(
      s"reader-frontier-width-${System.nanoTime()}",
      new RpcEndpoint {
        override val rpcEnv: RpcEnv = sc.env.rpcEnv
    ...
    groupProperties.setProperty(SparkContext.SPARK_PIPELINED_RUN_EPOCH, "reader-width-group")
    ...
    val initial = taskScheduler.resourceOffers(
      IndexedSeq(new WorkerOffer(executorId, "host0", 4))).flatten
    assert(initial.size === 2,
      "half of the executor must remain protected for a four-partition reader frontier")
    eventually(timeout(10.seconds)) {
      assert(prepared.synchronized(prepared.size) === 8)
    }
    prepared.synchronized(prepared.filter(_.shuffleId == 1).toSeq)
      .foreach(tracker.markInboxDrainReady(executorId, _))
    val upstream = taskScheduler.resourceOffers(
      IndexedSeq(new WorkerOffer(executorId, "host0", 2))).flatten
    assert(upstream.size === 4)
    val upstreamManager = taskScheduler.taskSetManagerForAttempt(1, 0).get
    val valueSer = SparkEnv.get.serializer.newInstance()
    upstream.take(3).foreach { task =>
      val result = new DirectTaskResult[Int](
        valueSer.serialize(task.index), Seq.empty, Array.emptyLongArray)
      taskScheduler.handleSuccessfulTask(upstreamManager, task.taskId, result)
    }

    assert(taskScheduler.resourceOffers(
      IndexedSeq(new WorkerOffer(executorId, "host0", 2))).flatten.isEmpty,
      "the three released slots must remain reserved until the next reader frontier is ready")
    prepared.synchronized(prepared.filter(_.shuffleId == 2).toSeq)
      .foreach(tracker.markInboxDrainReady(executorId, _))
    val downstream = taskScheduler.resourceOffers(
      IndexedSeq(new WorkerOffer(executorId, "host0", 2))).flatten
    assert(downstream.size === 4)
    assert(taskScheduler.taskSetManagerForAttempt(2, 0).get.runningTasks === 4)
  }
  ...
  test("prepared reader placement moves when its executor becomes excluded") {
    val taskScheduler = setupSchedulerWithMockTaskSetExcludelist(
      config.STREAMING_SHUFFLE_EXECUTOR_RECEIVE_SERVICE_ENABLED.key -> "true")
    val firstPrepared = new AtomicReference[StreamingShuffleReceiveInboxId]()
    val firstReleaseCount = new AtomicInteger(0)
    val secondPrepared = new AtomicReference[StreamingShuffleReceiveInboxId]()

    def endpoint(
        name: String,
        onPrepare: Seq[StreamingShuffleReceiveInboxId] => Unit,
        onRelease: () => Unit): RpcEndpointRef = {
      sc.env.rpcEnv.setupEndpoint(
        s"$name-${System.nanoTime()}",
        new RpcEndpoint {
          override val rpcEnv: RpcEnv = sc.env.rpcEnv
          ...
          override def receive: PartialFunction[Any, Unit] = {
            case ReleaseStreamingShuffleReceiveInbox(_) => onRelease()
          }
    ...
    assert(tracker.registerReceiveEndpoint(
      "executor0",
      endpoint("prepared-first", ids => {
        assert(ids.size === 1)
        firstPrepared.set(ids.head)
      }, () => firstReleaseCount.incrementAndGet())))
    assert(tracker.registerReceiveEndpoint(
      "executor1",
      endpoint("prepared-allowed", ids => {
        assert(ids.size === 1)
        secondPrepared.set(ids.head)
      }, () => ())))
    ...
    val oldInbox = firstPrepared.get()
    assert(oldInbox != null)
    assert(secondPrepared.get() == null)

    when(stageToMockTaskSetExcludelist(2).isExecutorExcludedForTaskSet("executor0"))
      .thenReturn(true)
    assert(taskScheduler.resourceOffers(offers).flatten.isEmpty)
    eventually(timeout(10.seconds)) {
      assert(firstReleaseCount.get() === 1)
    }
    val newInbox = secondPrepared.get()
    assert(newInbox != null)
    assert(newInbox !== oldInbox)
    assert(newInbox.taskAttemptId < oldInbox.taskAttemptId)
  ...
  test("decommissioning a receive executor reparents its prepared reader") {
    val taskScheduler = setupScheduler(
      config.STREAMING_SHUFFLE_EXECUTOR_RECEIVE_SERVICE_ENABLED.key -> "true")
    val firstPrepared = new AtomicReference[StreamingShuffleReceiveInboxId]()
    val firstReleaseCount = new AtomicInteger(0)
    ...
    def endpoint(
        name: String,
        prepared: AtomicReference[StreamingShuffleReceiveInboxId],
        onRelease: () => Unit): RpcEndpointRef = {
      sc.env.rpcEnv.setupEndpoint(
        s"$name-${System.nanoTime()}",
        new RpcEndpoint {
    ...
    assert(tracker.registerReceiveEndpoint(
      "executor0",
      endpoint("prepared-decommission-first", firstPrepared,
        () => firstReleaseCount.incrementAndGet())))
    assert(tracker.registerReceiveEndpoint(
      "executor1",
      endpoint("prepared-decommission-replacement", replacementPrepared, () => ())))
    ...
    val oldInbox = firstPrepared.get()
    assert(oldInbox != null)
    assert(replacementPrepared.get() == null)

    taskScheduler.executorDecommission(
      "executor0", ExecutorDecommissionInfo("planned decommission", None))
    assert(taskScheduler.resourceOffers(offers).flatten.isEmpty)
    eventually(timeout(10.seconds)) {
      assert(firstReleaseCount.get() === 1)
    }
  ...
  test("prepared receive mode expands a sole producer before reader compute attaches") {
    val taskScheduler = setupScheduler(
      config.STREAMING_SHUFFLE_EXECUTOR_RECEIVE_SERVICE_ENABLED.key -> "true",
      config.STREAMING_SHUFFLE_ELASTIC_PRODUCER_MAX_TASKS_PER_STAGE.key -> "2",
      config.STREAMING_SHUFFLE_ELASTIC_PRODUCER_SOLE_STAGE_MAX_TASKS.key -> "4")
    ...
    val prepared = new ArrayBuffer[StreamingShuffleReceiveInboxId]
    val endpoint = sc.env.rpcEnv.setupEndpoint(
      s"sole-frontier-prepared-${System.nanoTime()}",
      new RpcEndpoint {
        override val rpcEnv: RpcEnv = sc.env.rpcEnv
    ...
    val offers = IndexedSeq(new WorkerOffer(executorId, "host0", 8))
    val firstWave = taskScheduler.resourceOffers(offers).flatten
    assert(firstWave.length === 4)
    ...
    assert(taskScheduler.taskSetManagerForAttempt(1, 0).get.runningTasks === 0,
      "inbox preparation must not be confused with reader compute attachment")
    eventually(timeout(10.seconds)) {
      assert(prepared.synchronized(prepared.size) === 4)
    }
  }
  ...
  test("wide producer frontier becomes routable after every direct inbox is prepared") {
    val taskScheduler = setupScheduler(
      config.STREAMING_SHUFFLE_EXECUTOR_RECEIVE_SERVICE_ENABLED.key -> "true",
      config.STREAMING_SHUFFLE_ELASTIC_PRODUCER_MAX_TASKS_PER_STAGE.key -> "2",
      config.STREAMING_SHUFFLE_ELASTIC_PRODUCER_EXPANDED_MAX_TASKS_PER_STAGE.key -> "4",
    ...
    assert(taskScheduler.taskSetManagerForAttempt(4, 0).get.runningTasks === 0,
      "inbox preparation must not be confused with reader compute attachment")
    assert(taskScheduler.producerReaderFrontierRoutable(firstProducer),
      "every direct partition has a prepared executor-owned inbox")
  }
  ...
  test("prepared reader attach reserves capacity across a readiness race") {
    val taskScheduler = setupScheduler(
      config.STREAMING_SHUFFLE_EXECUTOR_RECEIVE_SERVICE_ENABLED.key -> "true",
      config.STREAMING_SHUFFLE_MAX_TOTAL_READER_TASKS_PER_EXECUTOR.key -> "1")
    val executorId = "executor0"
    val prepared = new ArrayBuffer[StreamingShuffleReceiveInboxId]
    val endpoint = sc.env.rpcEnv.setupEndpoint(
      s"reader-frontier-reservation-${System.nanoTime()}",
      new RpcEndpoint {
        override val rpcEnv: RpcEnv = sc.env.rpcEnv
    ...
    assert(tracker.registerReceiveEndpoint(executorId, endpoint))
    val groupProperties = new Properties()
    groupProperties.setProperty(SparkContext.SPARK_PIPELINED_RUN_EPOCH, "reader-race-group")
    ...
    val deepInbox = prepared.synchronized(prepared.find(_.shuffleId == 2).get)
    tracker.markInboxDrainReady(executorId, deepInbox)

    assert(taskScheduler.resourceOffers(offer).flatten.isEmpty,
      "the ready deeper reader must reserve the attachment for its unstarted dependency")
    assert(taskScheduler.taskSetManagerForAttempt(1, 0).get.runningTasks === 0)

    val upstreamInbox = prepared.synchronized(prepared.find(_.shuffleId == 1).get)
    tracker.markInboxDrainReady(executorId, upstreamInbox)
    assert(taskScheduler.resourceOffers(offer).flatten.size === 1)
    assert(taskScheduler.taskSetManagerForAttempt(2, 0).get.runningTasks === 1,
      "the upstream reader-producer must attach when it becomes ready")
    assert(taskScheduler.taskSetManagerForAttempt(1, 0).get.runningTasks === 0)
  }
