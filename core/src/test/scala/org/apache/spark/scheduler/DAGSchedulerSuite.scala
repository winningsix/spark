...
class DAGSchedulerSuite extends SparkFunSuite with TempLocalSparkContext with TimeLimits {
  ...
  // ==========================================================================================
  // Group-observable (coarse deferred) completion (M1.3)
  // ==========================================================================================
  ...
  test("pipelined shuffle: producer-then-consumer ordering completes normally (no deferral needed)") {
  ...
  test("pipelined shuffle: releasing a deferral is gated on the producer being genuinely done") {
    ...
    // consumers -- doing so would apply the consumer's results against soon-to-be-recomputed output.
    // Driving the exact bogus-epoch state through the mock backend is brittle, so we assert the
    // guard directly: releaseDeferredPipelinedConsumers is only reached from markStageAsFinished
    // when a ShuffleMapStage is available. Here we verify the retained-vs-released decision via the
    // observable deferral map across a normal producer completion (released) and confirm the
    // available-gate is the condition, complementing the by-inspection correctness of the guard.
    val producerRdd = new MyRDD(sc, 2, Nil)
    val pipelinedDep = new PipelinedShuffleDependency(producerRdd, new HashPartitioner(2))
    val consumerRdd = new MyRDD(sc, 2, List(pipelinedDep), tracker = mapOutputTracker)
    submit(consumerRdd, Array(0, 1))
    ...
    // Producer completes and IS available -> deferral released, consumer results applied.
    val producerStage =
  ...
  test("pipelined shuffle: full-group admission requires all 124 tasks in CPU reference DAG") {
    val leftProducerRdd = new MyRDD(sc, 60, Nil)
    val rightProducerRdd = new MyRDD(sc, 60, Nil)
    val myScheduler = scheduler.asInstanceOf[MyDAGScheduler]
    myScheduler.maxConcurrentTasksForTest = 6
    try {
      val leftDep = new PipelinedShuffleDependency(leftProducerRdd, new HashPartitioner(4))
      val rightDep = new PipelinedShuffleDependency(rightProducerRdd, new HashPartitioner(4))
      val readerRdd =
        new MyRDD(sc, 4, List(leftDep, rightDep), tracker = mapOutputTracker)
      val failure = new java.util.concurrent.atomic.AtomicReference[Exception]()
      val failListener = new JobListener {
        override def taskSucceeded(index: Int, result: Any): Unit = {}
        override def jobFailed(exception: Exception): Unit = failure.set(exception)
      ...
      submit(readerRdd, Array(0, 1, 2, 3), listener = failListener)

      assert(failure.get() != null,
        "full-group residency must reject a 124-task group on a 6-slot cluster")
      assert(failure.get().getMessage.contains("needs 124 concurrent task slots"))
      assert(failure.get().getMessage.contains("only 6 are currently free"))
      assertDataStructuresEmpty()
    } finally {
      myScheduler.maxConcurrentTasksForTest = 1000
    }
  ...
  test("pipelined shuffle: bounded CPU reference admits the 124-task DAG with 6 slots") {
    conf.set(config.SHUFFLE_MANAGER.key, classOf[ReaderResidencyRequiredShuffleManager].getName)
    ...
    try {
      ...
      submit(readerRdd, Array(0, 1, 2, 3), listener = listener)

      assert(listener.failure === null,
        "four resident readers plus one task from each producer fit exactly in six slots")
      assert(taskSets.size === 3,
        "both finite producer stages and the resident reader stage must be co-scheduled")
  ...
  // ==========================================================================================
  // Fail-fast on unsupported idioms in a pipelined group (M1.7, spec S9)
  // ==========================================================================================

  /** Submit `finalRdd` and return the job failure exception, or null if the job did not fail. */
  ...
  private def assertPipelinedUnsupported(failure: Exception, reasonSubstring: String): Unit = {
    ...
    assert(msg.contains(reasonSubstring), s"expected reason to mention '$reasonSubstring', got: $msg")
  }
  ...
  test("pipelined shuffle: a producer feeding more than one consumer (fan-out) is rejected") {
    // 1:N fan-out is a supported model deferred to a later version; v1 rejects it. To make the
    // producer feed two distinct consumer STAGES (narrow deps would collapse into one stage), each
    // consumer writes its own regular shuffle; a final stage reads both. Both consumer stages read
    // the same pipelined producer -> fan-out.
    val producerRdd = new MyRDD(sc, 2, Nil)
    val pipelinedDep = new PipelinedShuffleDependency(producerRdd, new HashPartitioner(2))
    ...
    val regularDepA = new ShuffleDependency(consumerA, new HashPartitioner(2))
    val regularDepB = new ShuffleDependency(consumerB, new HashPartitioner(2))
    val join = new MyRDD(sc, 2, List(regularDepA, regularDepB), tracker = mapOutputTracker)
    assertPipelinedUnsupported(submitAndCaptureFailure(join, Array(0, 1)), "more than one consumer")
    assertDataStructuresEmpty()
  }

  test("regular shuffle idioms are NOT rejected (inertness of the pipelined fail-fast)") {
    // None of the pipelined fail-fast checks may fire for a job with no pipelined dependency: a
    // barrier + indeterminate + reliable-checkpointed regular shuffle must run exactly as before.
    val producerRdd = new MyRDD(sc, 2, Nil, indeterminate = true)
    val regularDep = new ShuffleDependency(producerRdd, new HashPartitioner(2))
    val consumerRdd = new MyRDD(sc, 2, List(regularDep), tracker = mapOutputTracker)
    submit(consumerRdd, Array(0, 1))
