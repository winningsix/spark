...
class ShuffleDependencySuite extends SparkFunSuite with LocalSparkContext {
  ...
  test("PipelinedShuffleDependency never allows push-based shuffle merge") {
    ...
    // results for a transient shuffle. Even with push-based shuffle enabled cluster-wide -- where an
    // ordinary ShuffleDependency does allow merge -- a PipelinedShuffleDependency must not, so a
    // pipelined producer never acquires merger locations and its completion never routes through the
    // merge-finalize path in the DAGScheduler.
