...
package object config {
  ...
  private[spark] val SHUFFLE_MANAGER_INCREMENTAL =
    ConfigBuilder("spark.shuffle.manager.incremental")
      .doc("The ShuffleManager implementation used for pipelined (incrementally-readable) shuffle " +
        "dependencies, whose consumer stage may begin reading output while the producer stage is " +
        "still running. Regular shuffle dependencies continue to use spark.shuffle.manager. When " +
  ...
  private[spark] val SCHEDULER_PIPELINED_GROUP_PRODUCER_MAX_RUNNING_TASKS_PER_STAGE =
    ConfigBuilder("spark.scheduler.pipelined.group.producer.maxRunningTasksPerStage")
      ...
      .doc("Optional maximum number of concurrently running tasks for a pure producer stage in a " +
        ...
        "When unset or set to 0, pure producer stages have no per-stage cap. Pipelined task sets " +
        "are always scheduled round-robin.")
      .version("4.3.0")
      .intConf
  ...
  private[spark] val SCHEDULER_PIPELINED_GROUP_MAX_RUNNING_TASKS_PER_EXECUTOR =
    ConfigBuilder("spark.scheduler.pipelined.group.maxRunningTasksPerExecutor")
      .internal()
      .doc("Maximum number of concurrently running tasks from the same pipelined stage on a single " +
        "executor. A value of 0 disables this per-executor cap. When pipelined group round-robin " +
        "scheduling is enabled, Spark still prefers executors with fewer running tasks from the same " +
        "stage even when this cap is disabled, so one stage does not drain repeated offers into a " +
        "single executor.")
      .version("4.3.0")
      .intConf
      .checkValue(_ >= 0, "must be non-negative")
