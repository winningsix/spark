...
private[spark] class TaskSchedulerImpl(
  ...
  private val pipelinedShuffleTaskCoordinator =
    new PipelinedShuffleTaskCoordinator(conf, abortTimer, () => backend.reviveOffers())
  ...
  private[scheduler] def requiredPreTaskReaderInboxes(
    ...
    pipelinedShuffleTaskCoordinator.requiredReaderInboxes(taskSet, inboxes)
  }

  private[scheduler] def producerReaderFrontierRoutable(
      taskSet: TaskSetManager): Boolean = {
    pipelinedShuffleTaskCoordinator.readerFrontierRoutable(
      taskSet, rootPool.getSortedTaskSetQueue)
  }

  private[scheduler] def taskCpusForTaskSet(
      taskSet: TaskSet,
      profileTaskCpus: BigDecimal): BigDecimal = {
    pipelinedShuffleTaskCoordinator.taskCpus(taskSet, profileTaskCpus)
  }
  ...
  def initialize(backend: SchedulerBackend): Unit = {
    ...
    pipelinedShuffleTaskCoordinator.initialize()
    schedulableBuilder = {
  ...
  def taskSetFinished(manager: TaskSetManager): Unit = synchronized {
    ...
    pipelinedShuffleTaskCoordinator.removeTaskSet(manager)
          ...
          try {
            val (taskDescOption, didReject, index) =
              taskSet.resourceOffer(
                ...
                taskCpus,
                taskResAssignments,
                taskIndex => pipelinedShuffleTaskCoordinator.taskIndexAllowed(
                  taskSet, execId, taskIndex))
            if (taskDescOption.isDefined && index >= 0) {
              pipelinedShuffleTaskCoordinator.taskLaunched(taskSet, execId, index)
            }
  ...
  def resourceOffers(
    ...
    val runningPureProducersByRunEpoch =
    ...
    val startupFairProducerTaskLimitByRunEpoch =
      activePureProducerStagesByRunEpoch.collect { case (runEpoch, activeStages)
        ...
        val producerSlots = math.max(
          activeStages, freeCpuSlots +
            runningPureProducersByRunEpoch.getOrElse(runEpoch, 0) - reservedExecutors)
        runEpoch -> math.max(1, producerSlots / activeStages)
      }
 ...
 private def getTaskAccumulableInfosAndProcessRate(
  ...
  override def executorLost(executorId: String, reason: ExecutorLossReason): Unit = {
    ...
    synchronized {
      pipelinedShuffleTaskCoordinator.removeExecutor(executorId)
