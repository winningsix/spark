        ...
        if (stage.getNextAttemptId >= maxStageAttempts) {
          ...
          if (missing.isEmpty) {
            ...
            if (regularMissing.isEmpty && allPipelinedParentsRunning) {
              ...
              // Record that this stage is co-scheduled with still-running pipelined producers, so
              // its successful completions are deferred until those producers finish (S5).
              val deferral = pipelinedConsumerDeferrals.getOrElseUpdate(stage, DeferredCompletion())
