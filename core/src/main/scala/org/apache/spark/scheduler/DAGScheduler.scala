...
private[spark] class DAGScheduler(
   ...
   * must not be processed yet: doing so could finish the consumer's job and cancel the still-running
   * producer, or make the consumer's output observable before the producer's (spec S5,
   * group-observable completion). We buffer such events here and replay them once every pipelined
   * producer the consumer was waiting on has finished.
   *
  ...
  private[scheduler] val pendingPipelinedTaskSetsByGroup =
    new HashMap[String, ListBuffer[TaskSet]]
  ...
  private def pipelinedUnsupportedError(reason: String): SparkException = {
    new SparkException(s"PIPELINED_SHUFFLE_UNSUPPORTED: unsupported feature: $reason")
  }
   ...
   *    PipelinedShuffleDependency's producer is `dep.rdd`; a "consumer" is any RDD that lists that
   *    dependency. More than one distinct consumer RDD for the same pipelined shuffle is fan-out.
   *  - Reliable RDD checkpoint in a group member's within-stage chain (producer OR consumer side): a
   *    reliable `checkpoint()` writes a durable, lineage-truncated snapshot, which both reintroduces
   *    cross-time reuse of a transient edge (S4) and requires a post-success recompute of the
   *    member's transient input -- for a consumer, that input is the vanished pipelined shuffle.
   *    Checked here (not at stage creation) so a reject leaves no partial stage state, and so BOTH
   *    the producer chain (rooted at pd.rdd) and each consumer chain (rooted at a consuming RDD) are
   *    covered from the whole-graph view.
   *
  ...
  private def checkPipelinedGroupsSupportedInRDDGraph(finalRDD: RDD[_]): Unit = {
    // Distinct consumer RDDs per pipelined shuffleId, and the within-stage chain roots (producer
    // RDDs and consumer RDDs) whose chains must be free of reliable checkpoints.
    val consumersByShuffleId = new HashMap[Int, HashSet[Int]]
    val memberChainRoots = new HashSet[RDD[_]]
    traverseRDDGraph(finalRDD) { (rdd, enqueue) =>
      rdd.dependencies.foreach {
        case pd: PipelinedShuffleDependency[_, _, _] =>
          ...
          memberChainRoots += rdd      // consumer member: chain rooted at the reading RDD
          memberChainRoots += pd.rdd   // producer member: chain rooted at the produced RDD
          enqueue(pd.rdd)
        case dep =>
          enqueue(dep.rdd)
      }
    ...
    // Reject a reliable checkpoint anywhere in a member's within-stage chain (each root walked down
    // to the next shuffle boundary; keyed on checkpointData being ReliableRDDCheckpointData, not
    // isCheckpointed, since the write has not happened yet). Cache / .persist() / local checkpoint
    // are whole-partition and ephemeral and are not rejected.
    val hasReliableCheckpoint = memberChainRoots.exists { root =>
      !traverseParentRDDsWithinStage(root, (r: RDD[_]) =>
        !r.checkpointData.exists(_.isInstanceOf[ReliableRDDCheckpointData[_]]))
    }
    if (hasReliableCheckpoint) {
      throw pipelinedUnsupportedError(
  ...
  private def pipelinedReaderResidencyDemand(
      group: Set[Stage],
      policy: ReaderResidencyWithElasticProducers): PipelinedReaderResidencyDemand = {
    val demands = pipelinedReaderResidencyFrontiers(group).map {
      case (readers, producers) =>
        PipelinedReaderResidencyDemand(
          ...
          producerSlots =
            pipelinedReaderResidencyProducerSlots(
              producers, policy.minProducerTasksPerStage))
    }
    if (demands.isEmpty) {
  ...
  private def pipelinedReaderResidencyProducerSlots(
      producers: Seq[Stage],
      frontierTarget: Int): Int = {
    if (frontierTarget <= 1) {
      producers.size
  ...
  private def pipelinedReaderResidencyBlocked(
    ...
    val policy = pipelinedShuffleSchedulingRequirements(group).residencyPolicy match {
      case elastic: ReaderResidencyWithElasticProducers => elastic
      case _ => return None
    }

    val demand = pipelinedReaderResidencyDemand(group, policy)
  ...
  private def shouldQueuePipelinedTaskSetForPublication(
      isPipelinedMember: Boolean): Boolean = {
    pipelinedSubmitScopeDepth > 0 &&
      isPipelinedMember
  }

  private def queuePipelinedTaskSetForPublication(taskSet: TaskSet): Unit = {
    ...
    val pending = pendingPipelinedTaskSetsByGroup
      .getOrElseUpdate(groupId, new ListBuffer[TaskSet])
  ...
  /**
   * Publish only complete TaskSet groups. A group can become runnable across multiple DAGScheduler
   * events (for example, two pipelined producers behind different regular parents), so incomplete
   * groups remain buffered until every member TaskSet is present.
   */
  private def publishReadyPipelinedTaskSetGroups(): Unit = {
    val readyGroupIds = pendingPipelinedTaskSetsByGroup.iterator.collect {
      case (groupId, taskSets) if taskSets.nonEmpty &&
          taskSets.map(_.pipelinedGroupStageCount).distinct == Seq(taskSets.size) =>
        groupId
    }.toSeq.sorted

    readyGroupIds.foreach { groupId =>
      val taskSets = pendingPipelinedTaskSetsByGroup.remove(groupId).get.toSeq
      val requirements = taskSets.map(_.pipelinedGroupSchedulingRequirements).distinct
      if (requirements.size != 1) {
        throw SparkException.internalError(
          s"Pipelined group $groupId has inconsistent scheduling requirements: " +
            requirements.mkString("[", ", ", "]"))
      }
      taskScheduler.submitPipelinedGroup(taskSets, requirements.head)
    }
  }
  ...
  private def abortPipelinedShuffleGroup(stage: Stage, reason: String): Unit = {
    ...
    groupId.foreach { id =>
      ...
      if (registeredPipelinedShuffleGroups.contains(id) ||
          ...
          pendingPipelinedTaskSetsByGroup.contains(groupAttemptId)) {
  ...
  private def submitStage(stage: Stage): Unit = {
    ...
    try {
      ...
      pipelinedSubmitScopeDepth -= 1
      if (outermostPipelinedSubmitScope) {
        publishReadyPipelinedTaskSetGroups()
      }
    }
  }
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int): Unit = {
    ...
    if (tasks.nonEmpty) {
      ...
      val pipelinedSchedulingRequirements =
        if (isPipelinedMember) {
          pipelinedShuffleSchedulingRequirements(pipelinedGroup)
        } else {
          PipelinedGroupSchedulingRequirements()
        }
      val taskSet = new TaskSet(
        ...
        pipelinedGroupSchedulingRequirements = pipelinedSchedulingRequirements,
      ...
      if (shouldQueuePipelinedTaskSetForPublication(isPipelinedMember)) {
        queuePipelinedTaskSetForPublication(taskSet)
      } else {
          ...
          case smt: ShuffleMapTask =>
            ...
            if (!ignoreOldTaskAttempts) {
              ...
              if (shuffleStage.isPipelined) {
                ...
                //
                // Record the partition unconditionally, even for a "possibly bogus" epoch (a
                // success whose executor is already marked lost). The bogus-epoch guard exists only
                // to avoid trusting a MapOutputTracker registration that executor-loss stripping
                // could later remove; a pipelined stage never registers there and its completed set
                // is monotonic, so ignoring such a straggler would be actively harmful: pendingPartitions
                // was already decremented above, so a dropped last partition would leave the stage
                // "done but not available" and drive processShuffleMapStageCompletion to resubmit the
                // transient producer -- reopening SC-235532. A genuine mid-group failure is still
                // handled group-atomically (S6); an already-successful straggler is not a failure.
                shuffleStage.addPipelinedCompletedPartition(smt.partitionId)
              } else if (executorFailureEpoch.contains(execId) &&
